...
@section('title', 'Warehouse Management - Create Warehouse')
@section('content')
                        ...
                        <form action="{{ route('warehouse.store') }}" method="POST">
                            ...
                            <div class="form-group mb-3">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" id="warehouse_name" name="warehouse_name" class="form-control" placeholder="Enter Warehouse Name" value="{{ old('warehouse_name') }}" required>
                                </div>
                            ...
                            <div class="form-group mb-3">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" id="gst_no" name="gst_no" class="form-control" placeholder="Enter GST No." value="{{ old('gst_no') }}">
                                </div>
                            </div>
...
$(document).ready(function () {
    ...
    $('#state').on('change', function() {
        ...
        $('#city').empty().trigger('change'); // Clear city
        if (stateId) {
            $('#city').select2('open'); // Optional: auto-open city dropdown
        }
    });

    // === Preserve Old Values After Validation ===
    @if(old('state_id'))
        $('#state').val('{{ old('state_id') }}').trigger('change');
        setTimeout(function() {
            // Re-fetch cities and pre-select old city
            $.get('/get-cities/{{ old('state_id') }}', function(data) {
                const oldCityId = '{{ old('city_id') }}';
                $('#city').empty();
                data.forEach(city => {
                    const selected = city.id == oldCityId ? true : false;
