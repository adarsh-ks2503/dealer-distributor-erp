      ...
      @foreach ($selected_items as $item)
          ...
          cell5.innerHTML = `
          <input type="text" name="sauda_quantity[]" readonly value="{{ $item->sauda_quantity }}"
              class="form-control" id="sauda_quantity__${lastItemId}" >
          `;

          cell6.innerHTML = `
          <input type="text" readonly value="{{ $item->remaining }}" class="form-control"
              id="remmaining_sauda_quantity__${lastItemId}"  >
          `;

          cell7.innerHTML = `
          <input type="text" name="sauda_item_quantity[]" value="{{ $item->sauda_item_quantity }}"
              class="form-control" id="sauda_item_quantity__${lastItemId}" style="width: 150px!important"
              placeholder="0.00"
          ...
          cell9.innerHTML = `
          <input type="text" name="dis_quantity[]" value="{{ $item->dis_quantity }}" class="form-control"
              id="dis_quantity${lastItemId}" style="width: 150px!important" placeholder="0.000" required
          ...
          cell10.innerHTML = `
          <input type="text" name="sauda_negotiate_price[]" value="{{ $item->sauda_negotiate_price }}"
              class="form-control " id="sauda_negotiated_price_${lastItemId}" style="width: 150px!important"
              placeholder="0.000" value="{{ $item->negotiated_price ?? 0 }}" required
              oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\\..{2}).*$/, '$1').replace(/^0+([^\\.])/, '$1'); calculat_price(${lastItemId})">
          `;


          cell11.innerHTML = `
          <input type="text" name="loading_charge[]" value="{{ $item->loading_charge }}" class="form-control"
              id="loading_charge_${lastItemId}" style="width: 150px!important" placeholder="0.00"
              oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\\..{2}).*$/, '$1').replace(/^0+([^\\.])/, '$1'); calculat_price(${lastItemId})">
          `;

          cell12.innerHTML = `
          <input type="text" name="insurance[]" value="{{ $item->insurance }}" class="form-control"
              id="insurance_${lastItemId}" style="width: 150px!important" placeholder="0.00"
              oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\\..{2}).*$/, '$1').replace(/^0+([^\\.])/, '$1'); calculat_price(${lastItemId})">
          `;

          cell13.innerHTML = `
          <input type="text" name="commission[]" value="{{ $item->commission }}" class="form-control"
              id="commission_${lastItemId}" style="width: 150px!important" placeholder="0.00"
              oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\\..{2}).*$/, '$1').replace(/^0+([^\\.])/, '$1'); calculat_price(${lastItemId})">
          `;

          cell14.innerHTML = `
          <input type="text" name="over_billing[]" value="{{ $item->over_billing }}" class="form-control"
              id="over_billing_${lastItemId}" style="width: 150px!important" placeholder="0.00"
              oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\\..{2}).*$/, '$1').replace(/^0+([^\\.])/, '$1'); calculat_price(${lastItemId})">
          `;

          cell15.innerHTML = `
          <input type="text" name="final_price[]" value="{{ $item->final_price }}"
              style="width: 150px!important" class="form-control" id="final_price__${lastItemId}" placeholder="0.00"
              readonly>
          `;

          cell16.innerHTML = `
          <input type="text" name="total_price[]" value="{{ $item->total_price }}"
              style="width: 150px!important" class="form-control" id="total_price${lastItemId}" placeholder="0.00"
              readonly>
          `;
          ...
          <div>
              <select name="payment_type[]" class="form-select " id="sauda__payment_type${lastItemId}" required
                  ...
                  <option value="" disabled {{ !$item->payment_type ? 'selected' : '' }}>Select
                      Payment
                      Type</option>
                  <option value="Next Day" {{ $item->payment_type == 'Next Day' ? 'selected' : '' }}>Next
                      Day
                  </option>
                  <option value="Advanced" {{ $item->payment_type == 'Advanced' ? 'selected' : '' }}>
                      Advanced
                  </option>
                  <option value="Onloading" {{ $item->payment_type == 'Onloading' ? 'selected' : '' }}>
                      Onloading</option>
                  <option value="Other" {{ $item->payment_type == 'Other' ? 'selected' : '' }}>Other
                  </option>
              </select>
              <div id="sauda_item_other__type_input_${lastItemId}" class=" d-none ">
                  <input type="text" name="other_payment_type[]" value="{{ $item->other_payment_type }}"
                      class="form-control mt-1" placeholder="Other Payment">
              </div>
          ...
          cell18.innerHTML = `
          <input type="text" name="remark_item[]" value="{{ $item->remark_item }}"
              style="width: 250px!important" class="form-control" id="remark_item_${lastItemId}"
