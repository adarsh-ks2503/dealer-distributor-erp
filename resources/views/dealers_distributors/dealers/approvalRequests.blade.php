...
@section('title', 'Dealers Approval - Singhal')
@section('content')
    <main id="main" class="main">
        ...
        <div class="dashboard-header pagetitle">
            <h1>Dealers Approval Requests</h1>
            <nav>
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Dashboard</a></li>
                    <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Dealers</a></li>
                    <li class="breadcrumb-item active">Dealers Approval Requests</li>
                </ol>
            </nav>
        </div><!-- End Page Title -->
                        ...
                        <div class="card-body mt-4">
                            ...
                            <div class="row ">
                                <div class="col-md-6 col-sm-12">
                                    <div class="pd-20">
                                        <h4 class="text-blue h4">Dealers Approval Requests</h4>
                                    </div>
                                </div>
                            ...
                            <table class="display stripe row-border order-column" id="dlr_approval_req" style="width:100%">
                                <thead>
                                    <tr>
                                        <th class="text__left">#</th>
                                        <th class="text__left">DEALER NAME</th>
                                        <th class="text__left">DEALER CODE</th>
                                        <th class="text__left">ORDER LIMIT</th>
                                        <th class="text__left">MOBILE NUMBER</th>
                                        <th class="text__left">STATUS</th>
                                        <th class="text__left">STATUS CHANGE TIME</th>
                                        <th class="text__left">ACTION</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach ($dealers as $index => $item)
                                        <tr>
                                            <td class="text__left">{{ $index + 1 }}</td>
                                            <td class="text__left">{{ $item->name }}</td>
                                            <td class="text__left">{{ $item->code }}</td>
                                            <td class="text__left">{{ $item->order_limit }}</td>
                                            <td class="text__left">{{ $item->mobile_no }}</td>
                                            <td class="text__left">
                                                @php
                                                    $statusClasses = [
                                                        'pending' => 'bg-warning',
                                            ...
                                            <td class="text__left">{{ $item->updated_at->format('j F, Y h:i A') }}</td>
                                                        ...
                                                        <ul class="dropdown-menu custom-dropdown-menu" aria-labelledby="actionMenu{{ $item->id }}">
                                                            <li>
                                                                <button class="dropdown-item approve-btn"
                                                                        data-id="{{ $item->id }}"
                                                                        data-url="{{ route('dealers.approve', $item->id) }}">
                                                            ...
                                                            @if ($item->status === 'Pending')
                                                                <li>
                                                                    <button class="dropdown-item reject-btn"
                                                                            data-id="{{ $item->id }}"
                                                                            data-url="{{ route('dealers.reject', $item->id) }}">
                                                                ...
                                                                @if ($item->status != 'Inactive')
                                                                    <li>
                                                                        <a class="dropdown-item"
                                                                            href="{{ route('dealers.edit', $item->id) }}"><i
                                                            ...
                                                            @can('Dealers-View')
                                                                <li><a class="dropdown-item"
                                                                        href="{{ route('dealers.show', $item->id) }}"><i
            ...
            <form id="approveForm" method="POST">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        Are you sure you want to <strong>approve</strong> this Dealer?
                    </div>
        ...
        <div class="modal-dialog">
            <form id="rejectForm" method="POST">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        Are you sure you want to <strong>reject</strong> this Dealer?
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
