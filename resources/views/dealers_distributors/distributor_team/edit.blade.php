...
@section('title', 'Edit Team - Singhal')
@section('content')
    ...
    <section class="section">
        <div class="card shadow-sm">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h4 class="mb-0 fw-bold">Edit Team</h4>
                <a href="{{ route('distributor_team.index') }}" class="btn btn-outline-dark">
                    <i class="bi bi-arrow-left-circle me-1"></i> Back
                </a>
            </div>

            <form action="{{ route('distributor_team.update', $team->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="card-body mt-3">
                    ...
                    <div class="row g-4">
                        <div class="col-md-6">
                            ...
                            <select name="distributor_id" id="distributor_id" class="form-select custom-input" required disabled>
                                <option value="">Select</option>
                                @foreach ($distributors as $distributor)
                                    <option value="{{ $distributor->id }}"
                                        {{ $distributor->id == $team->distributor_id ? 'selected' : '' }}>
                                        {{ $distributor->name }}
                                    </option>
                                @endforeach
                            </select>
                            <input type="hidden" name="distributor_id" value="{{ $team->distributor_id }}">
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="number" name="distributor_team_size" class="form-control custom-input" readonly value="{{ $team->no_of_dealers }}">
                        </div>

                        <div class="col-md-6">
                            <label class="form-label fw-semibold">Distributor Individual Order Limit (MT):</label>
                            <input type="number" name="distributor_individual_order_limit" class="form-control custom-input" readonly value="{{ $team->distributor->order_limit }}">
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="number" name="distributor_order_limit" class="form-control custom-input" readonly value="{{ $team->total_order_limit }}">
                        </div>


                        <div class="col-md-12">
                            <label class="form-label fw-semibold">Remark:</label>
                            <textarea name="remark" id="" cols="30" rows="2" class="form-control @error('remark') is-invalid @enderror" >{{ $team->remarks }}</textarea>
                        </div>
                    </div>
                </div>

                <div class="card-footer d-flex justify-content-end gap-2">
                    <a href="{{ route('distributor_team.index') }}" class="btn btn-secondary">Cancel</a>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    let selectedDealers = @json(
        $selectedDealers
            ->filter(fn($d) => optional($d->pivot)->status === 'Active')
            ->map(fn($d) => ['id' => $d->id, 'name' => $d->name])
            ->values()
    );
    let distributorIndividualLimit = parseFloat(distributorIndividualLimitInput.value) || 0;
    ...
    // --- Initial Load ---
    function initialize() {
        const initialDistributorId = distributorSelect.value;
        if (initialDistributorId) {
            loadAvailableDealers(initialDistributorId);
        }
        renderDealers(); // Render initially selected dealers
    }

    // --- Core Functions ---
    async function loadAvailableDealers(distributorId) {
        if (!distributorId) {
            availableDealers = [];
            renderDealers();
            return;
        ...
        try {
            const response = await fetch(`/get-dealers-by-distributor/${distributorId}`);
            const allDealers = await response.json();
            const selectedIds = new Set(selectedDealers.map(d => d.id));
            availableDealers = allDealers.filter(d => !selectedIds.has(d.id));
            renderDealers();
        } catch (error) {
    ...
    async function handleDistributorChange() {
        ...
        // When distributor is changed, all previous selections are cleared.
        selectedDealers = [];
        availableDealers = [];
        ...
        distributorTotalLimitInput.value = '';
        dealerOrderLimitInput.value = '';

        if (distributorId) {
            try {
                const response = await fetch(`/get-distributor/${distributorId}`);
                ...
                await loadAvailableDealers(distributorId); // This will call renderDealers
            } catch (error) {
    ...
    function createDealerItem(dealer, action) {
        ...
        div.innerHTML = `<span>${dealer.name}</span><i class="fas fa-${action === 'add' ? 'plus' : 'times'}"></i>`;
